...
The final-answer LLM is user-selectable per query; query rewriting and conversation summarization always use the default Gemini generator (`rag_factory.py::build_generator()`) regardless of what's selected. `rag/llm/protocol.py::TextGenerator` is a structural Protocol (`generate(prompt) -> LLMResponse`) that `GeminiGenerator` and `GroqGenerator` (`rag/llm/groq_generator.py`, using the `groq` SDK against Groq's OpenAI-compatible chat completions API) both satisfy without inheriting from it — the same seam used for `ConversationStore`/`ChunkSink`. `HistoryAwareRAGService.answer_with_sources`/`answer` take an optional `generator: TextGenerator | None` that overrides `self.generator` for that call only.

The catalog of offered models — id, display label, provider, and (for Groq) the API model string — lives in `backend/wiring/rag_factory.py::_GENERATION_MODEL_SPECS`, alongside `available_generation_models()` (filters out Groq entries when `GROQ_API_KEY` is unset — they're omitted, not listed-and-failing) and `resolve_generator(model_id)` (returns `(generator, resolved_id)`, raising `UnknownGenerationModelError` for an unknown or currently-unavailable id rather than silently substituting a different model than what was asked for). Both are exposed to routers only through `backend/dependencies.py` (`GenerationModels`, `DefaultGenerationModelId`, `GeneratorResolver`) rather than imported directly, so API tests can override them with stubs instead of needing real `GEMINI_API_KEY`/`GROQ_API_KEY` credentials — mirrors how `get_rag_service` is stubbed. `GET /api/models` (authenticated) lists the current options; `POST /api/query` accepts an optional `model` field (id from that list; omitted or `null` uses the default) and echoes back which model actually answered in the response's `model` field. Selection is not persisted server-side — like `top_k`, it's resent by the client on every request (the frontend remembers the last choice in `localStorage`).
